...
class Tracer {
    public:
        ...
        void Screen(TraceResult& input, const std::vector<int>& thresholds, const bool& categorize);
        std::string Categorize(const std::vector<cv::Point>& approxContour);
        cv::Point GetCentroid(const cv::Rect& bbox);
    
        //cv::Mat Markup(const cv::Mat& image, std::vector<std::vector<cv::Point>> contours);
    };